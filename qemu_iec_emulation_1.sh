...
qemu-system-arm \
  ...
  -kernel ./vms/zImage \
  -bios ./vms/u-boot.bin\
  -drive if=none,file=./vms/vm1_rootfs.ext2,format=raw,id=hd0 \
  ...
  -device virtio-net-pci,netdev=net0,mac=ee:7c:ad:d5:1d:63\
  -chardev socket,id=i2c0,path=/tmp/vu_i2c0-0\
